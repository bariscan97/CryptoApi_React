...
import Charjs from '../components/Charjs'
...
const Coin = () => {
    ...
    const [valuess,setvaluess]=useState({last:[],volume:[]})
    ...
    useEffect(() => {
        axios.get(url).then((res) => {
            ...
            for (let i=0;i<res.data.tickers.length;i++){
                arr1.push(res.data.tickers[i].last)
                arr2.push(res.data.tickers[i].volume)
               
            }
            setvaluess({last:[...arr1],volume:[...arr2]})
